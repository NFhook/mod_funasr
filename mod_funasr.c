/*
 * mod_funasr FreeSWITCH module to stream audio to websocket and receive response
 */
#include "mod_funasr.h"
#include "audio_streamer_glue.h"

SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_funasr_shutdown);
SWITCH_MODULE_RUNTIME_FUNCTION(mod_funasr_runtime);
SWITCH_MODULE_LOAD_FUNCTION(mod_funasr_load);

SWITCH_MODULE_DEFINITION(mod_funasr, mod_funasr_load, mod_funasr_shutdown, NULL /*mod_funasr_runtime*/);
                                     ...
                                     char* metadata)
{
    ...
    if (switch_channel_get_private(channel, MY_BUG_NAME)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_funasr: bug already attached!\n");
    ...
    if (switch_channel_pre_answer(channel) != SWITCH_STATUS_SUCCESS) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_funasr: channel must have reached pre-answer status before calling start!\n");
        return SWITCH_STATUS_FALSE;
    }
    ...
    if (SWITCH_STATUS_FALSE == stream_session_init(session, responseHandler, read_codec->implementation->actual_samples_per_second,
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Error initializing mod_funasr session.\n");
        return SWITCH_STATUS_FALSE;
    }
...
static switch_status_t do_stop(switch_core_session_t *session, char* text)
{
    ...
    if (text) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_funasr: stop w/ final text %s\n", text);
    }
    else {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_funasr: stop\n");
    }
...
static switch_status_t do_pauseresume(switch_core_session_t *session, int pause)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_funasr: %s\n", pause ? "pause" : "resume");
...
static switch_status_t send_text(switch_core_session_t *session, char* text) {
    ...
    if (bug) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_funasr: sending text: %s.\n", text);
        status = stream_session_send_text(session, text);
    }
    else {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_funasr: no bug, failed sending text: %s.\n", text);
    }
    return status;
}
...
SWITCH_STANDARD_API(stream_function)
{
    ...
    assert(cmd);
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "mod_funasr cmd: %s\n", cmd ? cmd : "");
...
SWITCH_MODULE_LOAD_FUNCTION(mod_funasr_load)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_funasr API loading..\n");
...
/*
  ...
  Macro expands to: switch_status_t mod_funasr_shutdown() */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_funasr_shutdown)
{
